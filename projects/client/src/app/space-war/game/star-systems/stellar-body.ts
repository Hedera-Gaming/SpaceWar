...
export class StellarBody
    implements Updatable, HasGameObject<Phaser.GameObjects.Sprite>
{
    readonly id: string;
    active: boolean;
    ...
    static preload(scene: Phaser.Scene): void {
        ...
        scene.load.spritesheet(
            'asteroids',
            `${environment.baseUrl}/assets/tiles/asteroids-tile.png`,
            {
                frameWidth: 100,
                frameHeight: 100,
                startFrame: 0,
    ...
    constructor(scene: Phaser.Scene, options: StellarBodyOptions) {
        ...
        this.rotationSpeed =
            NumberOrRange.getRealNumber(options.rotationSpeed) ??
            Phaser.Math.RND.realInRange(0.1, 1);
        this.scale =
            NumberOrRange.getRealNumber(options.scale) ??
            Phaser.Math.RND.realInRange(0.1, 3);
        this.scrollFactor =
            NumberOrRange.getRealNumber(options.scrollFactor) ??
            Phaser.Math.RND.realInRange(0.05, 0.5);
    ...
    private _createGameObj(): void {
        ...
        this._gameObj.setDepth(SpaceWarClient.Constants.UI.Layers.STELLAR);
