import { TryCatch } from 'space-war-shared';
...
export class Radar extends Camera {
    ...
    protected override _getCamera(
        ...
        const cam = super._getCamera({
            camera:
                opts.camera ??
                this.scene.cameras.add(0, 0, opts.width, opts.height, false),
            ...opts,
            x: opts.x - radius,
            y: opts.y - opts.height / 2,
        });
        this._mask = this.scene.make
            .graphics({ x: opts.x, y: opts.y }, false)
    ...
    override destroy(): void {
        ...
        TryCatch.run(
            ...
            'warn',
            'unable to cleanly remove Radar'
        );
    }
}
