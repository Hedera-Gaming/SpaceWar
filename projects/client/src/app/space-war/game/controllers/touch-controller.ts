...
export class TouchController extends InputController {
    ...
    private _handleAimTouch(x: number, y: number): void {
        const pos: Phaser.Math.Vector2 = Helpers.vector2(x, y).subtract(
            Helpers.vector2(60)
        );
        const radians: number = Phaser.Math.Angle.BetweenPoints(
            pos,
            Helpers.vector2()
        );
        const degrees: number = Number(Helpers.rad2deg(radians).toFixed(0));
        const ship = this.parentScene?.getShip?.(SpaceWarClient.playerShipId);
        if (ship) {
    ...
    private _createLeftStick(): LayoutContent {
        ...
        const leftStick: Phaser.GameObjects.Arc = this.scene.add
            .circle(0, 0, radius, 0xf0f0f0, 0.2)
    ...
    private _createThrowButton(): LayoutContent {
        const throwButton = new TextButton(this.scene, {
            ...
            backgroundStyles: {
                fillStyle: { color: 0x000000, alpha: 0.3 },
                lineStyle: { color: 0xffff00, width: 2 },
            },
            onClick: () => {
                throwButton
                    .setText({ style: { color: '#000000' } })
