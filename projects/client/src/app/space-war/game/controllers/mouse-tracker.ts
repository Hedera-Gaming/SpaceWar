...
import { HasLocation, Updatable, SpaceWar, Helpers } from 'space-war-shared';
...
export class MouseTracker implements HasLocation, Updatable {
    ...
    get location(): Phaser.Types.Math.Vector2Like {
        ...
        return this._pointer()?.positionToCamera(
        ...
        if (this._scene) {
            ...
            if (
                !Phaser.Math.Fuzzy.Equal(
                    this._ship.rotationContainer.angle,
                    degrees,
                    SpaceWar.Constants.Ships.MIN_ROTATION_ANGLE
                )
            ) {
                SpaceWarClient.socket?.sendSetShipAngleRequest(
                    degrees,
