...
export class KbmController extends InputController {
    ...
    get pointerLocation(): Phaser.Types.Math.Vector2Like {
        const pos = this.pointer?.positionToCamera(
            this.parentScene?.cameras?.main
        );
        if (pos && typeof pos === 'object' && 'x' in pos && 'y' in pos) {
            return pos as Phaser.Types.Math.Vector2Like;
        }
        return { x: 0, y: 0 };
    }
    ...
    update(time: number, delta: number): void {
        if (this.active) {
            ...
            if (
                ...
                const ship = this.parentScene?.getShip?.(
                ...
                if (ship) {
                    ...
                    const degrees: number = Number(
                        Helpers.rad2deg(radians).toFixed(0)
                    );
                    // only update if angle changed more than minimum allowed degrees
                    if (
                        !Phaser.Math.Fuzzy.Equal(
                            ship.rotationContainer.angle,
                            degrees,
                            SpaceWar.Constants.Ships.MIN_ROTATION_ANGLE
                        )
                    ) {
    ...
    private _setupInputHandling(): void {
        this._thrustForwardsKey = this.scene.input.keyboard.addKey(
            'SPACE',
            true,
            true
        );
        // this._thrustForwardsKey = this.scene.input.keyboard.addKey('W', true, true);
        this._thrustBackwardsKey = this.scene.input.keyboard.addKey(
            'S',
            true,
            true
        );
        this._strafeLeftKey = this.scene.input.keyboard.addKey('A', true, true);
        this._strafeRightKey = this.scene.input.keyboard.addKey(
            'D',
            true,
            true
        );
        this._boostKey = this.scene.input.keyboard.addKey('TAB', true, false);
        this._rotateAttachmentsClockwiseKey = this.scene.input.keyboard.addKey(
            'E',
            true,
            false
        );
        this._rotateAttachmentsAntiClockwiseKey =
            this.scene.input.keyboard.addKey('Q', true, false);
        this._detachAttachmentKey = this.scene.input.keyboard.addKey(
            'X',
            true,
            false
        );
        this._throwAttachmentKey = this.scene.input.keyboard.addKey(
            'T',
            true,
            false
        );
        this._grabAttachmentKey = this.scene.input.keyboard.addKey(
            'G',
            true,
            false
        );
