...
export class PlayerShip extends Ship {
    ...
    static preload(scene: Phaser.Scene): void {
        scene.load.image(
            'weapons-1',
            `${environment.baseUrl}/assets/sprites/ship-parts/weapons-1.png`
        );
        scene.load.image(
            'weapons-2',
            `${environment.baseUrl}/assets/sprites/ship-parts/weapons-2.png`
        );
        scene.load.image(
            'weapons-3',
            `${environment.baseUrl}/assets/sprites/ship-parts/weapons-3.png`
        );
        scene.load.image(
            'wings-1',
            `${environment.baseUrl}/assets/sprites/ship-parts/wings-1.png`
        );
        scene.load.image(
            'wings-2',
            `${environment.baseUrl}/assets/sprites/ship-parts/wings-2.png`
        );
        scene.load.image(
            'wings-3',
            `${environment.baseUrl}/assets/sprites/ship-parts/wings-3.png`
        );
        scene.load.image(
            'cockpit-1',
            `${environment.baseUrl}/assets/sprites/ship-parts/cockpit-1.png`
        );
        scene.load.image(
            'cockpit-2',
            `${environment.baseUrl}/assets/sprites/ship-parts/cockpit-2.png`
        );
        scene.load.image(
            'cockpit-3',
            `${environment.baseUrl}/assets/sprites/ship-parts/cockpit-3.png`
        );
        scene.load.image(
            'engine-1',
            `${environment.baseUrl}/assets/sprites/ship-parts/engine-1.png`
        );
        scene.load.image(
            'engine-2',
            `${environment.baseUrl}/assets/sprites/ship-parts/engine-2.png`
        );
        scene.load.image(
            'engine-3',
            `${environment.baseUrl}/assets/sprites/ship-parts/engine-3.png`
        );
        scene.load.image(
            'minimap-player',
            `${environment.baseUrl}/assets/sprites/minimap-player.png`
        );
        scene.load.image(
            'overheat-glow',
            `${environment.baseUrl}/assets/particles/red-glow.png`
        );
    }
    ...
    override subtractIntegrity(
        amount: number,
        damageOpts: DamageMetadata
    ): void {
        ...
        if (this.active && amount !== 0) {
            ...
            if (!this._shipDamageFlicker?.isPlaying()) {
                this._shipDamageFlicker = Animations.flicker(
    ...
    private _addVisualsToGameObject(options: ShipOptions): void {
        this.setDepth(SpaceWarClient.Constants.UI.Layers.PLAYER);
    ...
    private _createOverheatIndicator(): void {
        this._shipOverheatIndicator = this.scene.add.text(0, -75, 'OVERHEAT', {
            font: '30px Courier',
            color: '#ffff00',
            stroke: '#ff0000',
            strokeThickness: 4,
        ...
        this._shipOverheatIndicator.setX(
            -this._shipOverheatIndicator.width / 2
        );
    ...
    private _createRadarSprite(): void {
        ...
        this.add(this._radarSprite).bringToTop(this._radarSprite);
    }

    private _createSelfDestructText(): void {
        this._selfDestructText = this.scene.make
            .text({
                x: 0,
                y: -75,
                text: `3`,
    ...
    private _updateOverheatingSpriteAndText(): void {
        if (this.active) {
            const alpha =
                this.temperature /
                SpaceWar.Constants.Ships.MAX_SAFE_TEMPERATURE;
            this._shipHeatIndicator.setAlpha(Math.min(alpha, 1));

            if (this.isOverheating) {
                if (
                    !this.scene.tweens.getTweensOf(this._shipOverheatIndicator)
                        ?.length
                ) {
    ...
    private _updateIntegrityIndicator(): void {
        if (this.active && this._shipIntegrityIndicator) {
            ...
            let square: Phaser.GameObjects.Graphics = this.scene.add.graphics({
                fillStyle: { color: 0xff6060 },
            });
            square.fillRect(
                -Math.floor(this.integrity / 2),
                -2,
                this.integrity,
                4
            );
            let squareContainer: Phaser.GameObjects.Container =
                this.scene.add.container(0, 0, [square]);
    ...
    private _updateSelfDestructText(): void {
        if (this.active) {
            if (this.destroyAtTime) {
                const remainingTime =
                    this.destroyAtTime - this.scene.game.getTime();
                const remainingSeconds = Math.ceil(remainingTime / 1000);

                this._selfDestructText
                    .setText(`${remainingSeconds}`)
                    .setAlpha(1);
            } else {
