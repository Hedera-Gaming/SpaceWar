...
export class PlayerBullet extends Bullet {
    ...
    static preload(scene: Phaser.Scene): void {
        scene.load.image(
            'bullet',
            `${environment.baseUrl}/assets/sprites/bullet.png`
        );
        scene.load.audio(
            'cannon-fire',
            `${environment.baseUrl}/assets/audio/effects/cannon-fire.ogg`
        );
        scene.load.audio(
            'bullet-hit',
            `${environment.baseUrl}/assets/audio/effects/bullet-hit.ogg`
        );
    }
    ...
    private _addUiElements(): void {
        ...
        const glow = this.scene.add.sprite(
            ...
            0,
            'flares',
            SpaceWarClient.Constants.UI.SpriteMaps.Flares.green
        );
        ...
        this.setDepth(SpaceWarClient.Constants.UI.Layers.PLAYER).add([
            ball,
            glow,
        ]);
        TryCatch.run(
            () => (this._fireSound = this.scene.sound.add('cannon-fire')),
            'warn'
        );
        TryCatch.run(
