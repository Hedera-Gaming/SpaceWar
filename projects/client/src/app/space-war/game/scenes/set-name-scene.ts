...
import { Sanitiser, SpaceWar, TryCatch } from 'space-war-shared';
...
export class SetNameScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (SpaceWar.UserData.isValid(SpaceWarClient.playerData)) {
            this._text
                .contentAs<Phaser.GameObjects.Text>()
                .setText(SpaceWarClient.playerData.name);
    ...
    private _getMobileTextInput(): void {
        this._text
            ...
            .on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, () => {
                const txt: string = window.prompt(
                    'Enter player name: (minimum 3 characters consisting of [a-zA-Z0-9])'
                );
    ...
    private _validateAndStartGame(text: string): void {
        ...
        if (pname.length > 2) {
            SpaceWarClient.socket.sendSetPlayerDataRequest({
    ...
    private _updateContinueButton(): void {
        if (this._button.contentAs<Phaser.GameObjects.Text>().text.length > 2) {
            ...
            this._button
                .setText({ style: style.text })
                .setBackground(style.graphics)
                .setOnClick(() => {
                    this._validateAndStartGame(
                        this._text.contentAs<Phaser.GameObjects.Text>().text
                    );
                });
        } else {
            this._button
                .setText({ style: this._continueButtonStylesDisabled.text })
