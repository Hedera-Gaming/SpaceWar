...
export class GameplayHudScene extends Phaser.Scene implements Resizable {
    ...
    create(): void {
        this._parentScene = SpaceWar.game.scene.getScene(
            GameplaySceneConfig.key
        ) as BaseScene;
        this.resize();
        SpaceWar.stats.start(this.playerShip.currentState);
    }
    ...
    get playerShip(): Ship {
        return this._parentScene.getShip(SpaceWarClient.playerShipId);
    }
    ...
    private _createHUD(): void {
        ...
        this._destructButton = new TextButton(
            this,
            TextButtonOptions.Outline.warning({
                textConfig: { text: 'SELF DESTRUCT' },
                padding: 5,
                cornerRadius: 5,
                onHover: () => {
                    this._destructButton
                        .setText({ style: Styles.warning().text })
                        .setBackground(Styles.warning().graphics);
                },
                onClick: () => {
                    this.playerShip.selfDestruct();
                    this._quitContainer.removeContent(false);
                    this._cancelDestructButton.setActive(true).setVisible(true);
                    this._quitContainer.setContent(this._cancelDestructButton);
                    this._destructButton.setActive(false).setVisible(false);
                },
            })
        );
        ...
        this._cancelDestructButton = new TextButton(
            this,
            TextButtonOptions.Outline.danger({
                textConfig: { text: 'CANCEL' },
                padding: 5,
                cornerRadius: 5,
                onHover: () => {
                    this._cancelDestructButton
                        .setText({ style: Styles.danger().text })
                        .setBackground(Styles.danger().graphics);
                },
                onClick: () => {
                    this.playerShip.cancelSelfDestruct();
                    this._quitContainer.removeContent(false);
                    this._destructButton.setActive(true).setVisible(true);
        ...
        this._hudLayout = new GridLayout(this, {
            ...
            .setDepth(SpaceWarClient.Constants.UI.Layers.HUD);
    ...
    private _createController(): void {
        if (this.game.device.os.desktop) {
            ...
            if (this._controller) {
                (this._controller as TouchController)
                    .getGameObject()
                    ?.destroy();
            }
        ...
        this.parentScene.events
            ...
            .on(
                SpaceWar.Constants.Events.SHIP_ANGLE,
                (id: string, degrees: number) => {
                    this.parentScene
                        .getShip(id)
                        .rotationContainer.setAngle(degrees);
                }
            );
    }
    ...
    private _displayHUDInfo(): void {
        TryCatch.run(() => {
            ...
            const stats = SpaceWar.stats.getStats({ shipId: id });
            const info: string[] = [
                `Elapsed: ${((Date.now() - stats?.startedAt) / 1000).toFixed(
                    1
                )}`,
                `Enemies: ${stats?.opponentsDestroyed?.length ?? 0}/${
                    SpaceWarClient.opponents.length
                }`,
                `Fuel: ${this.playerShip.remainingFuel.toFixed(1)}`,
                `Ammo: ${this.playerShip.weapon.remainingAmmo || 0}`,
                `Score: ${SpaceWar.stats.getScore(id).toFixed(0)}`,
            ];
            if (Logging.shouldLog('debug')) {
                const loc = this.playerShip.location;
                info.push(`Location: ${loc.x.toFixed(1)},${loc.y.toFixed(1)}`);
                info.push(
                    `Angle: ${this.playerShip.rotationContainer.angle.toFixed(
                        1
                    )}`
                );
            }
