...
export class MultiplayerHudScene extends Phaser.Scene implements Resizable {
    ...
    update(time: number, delta: number): void {
        this._connectedToServer = SpaceWarClient.socket?.connected ?? false;
        ...
        this.parentScene.events
            .on(
                ...
                (id: string, enabled: boolean) => {
                    SpaceWarClient.socket?.sendEngineOnRequest(enabled);
                }
            )
            .on(
                SpaceWar.Constants.Events.WEAPON_FIRING,
                (id: string, firing: boolean) => {
                    SpaceWarClient.socket?.sendWeaponFiringRequest(firing);
                }
            )
            .on(
                SpaceWar.Constants.Events.SHIP_ANGLE,
                (id: string, degrees: number) => {
                    SpaceWarClient.socket?.sendSetShipAngleRequest(degrees);
                }
            );
    }
    ...
    private _displayHUDInfo(): void {
        try {
            const stats = SpaceWar.stats.getStats({
            ...
            const info: string[] = [
                ...
                `Score: ${SpaceWar.stats
                    .getScore(this.playerShip.id)
                    .toFixed(0)}`,
            ];
