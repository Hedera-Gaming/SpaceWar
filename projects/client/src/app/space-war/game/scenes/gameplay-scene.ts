...
export class GameplayScene extends BaseScene implements Resizable {
    ...
    get playerShip(): PlayerShip {
        return this.getShip<PlayerShip>(SpaceWarClient.playerShipId);
    }
    ...
    create(): void {
        ...
        SpaceWar.game.scene.start(GameplayHudSceneConfig.key);
        SpaceWar.game.scene.bringToTop(GameplayHudSceneConfig.key);

        this.resize();

        this.addRepeatingAction(
            'high',
            'update-ships',
            ...
            .addRepeatingAction(
                ...
                (time: number, delta: number) => {
                    const activeEnemies = SpaceWarClient.opponents.filter(
                        (o) => o?.ship?.active
                    );
            ...
            .addRepeatingAction('medium', 'reveal-room', () => {
                ...
                const currentRoom = this.getLevel().getRoomAtWorldXY(
                    currentLoc.x,
                    currentLoc.y
                );
    ...
    private _createOpponents(): void {
        ...
        SpaceWarClient.opponents.splice(0, SpaceWarClient.opponents.length);

        // add opponent in each room
        for (var room of this.getLevel().rooms) {
            let tl: Phaser.Math.Vector2 =
                this.getLevel().getMapTileWorldLocation(
                    room.left + 1,
                    room.top + 1
                );
            let br: Phaser.Math.Vector2 =
                this.getLevel().getMapTileWorldLocation(
            ...
            this.physics.add.collider(p, this.getLevel().wallsLayer, () => {
                const factor = SpaceWar.Constants.Ships.WALL_BOUNCE_FACTOR;
                p.body.velocity.multiply({ x: factor, y: factor });
            });
            let controller = new ClientAiController(this, p).setEnemyIds(
                SpaceWarClient.playerShipId
            );
            SpaceWarClient.opponents.push(controller);
    ...
    private _createMapAndPlayer(): void {
        ...
        const startTopLeft: Phaser.Math.Vector2 =
            this.getLevel().getMapTileWorldLocation(
                startingRoom.left + 1,
                startingRoom.top + 1
            );
        const startBottomRight: Phaser.Math.Vector2 =
            this.getLevel().getMapTileWorldLocation(
                startingRoom.left + startingRoom.width - 1,
                startingRoom.top + startingRoom.height - 1
            );
        ...
        SpaceWarClient.playerShipId = ship.id;
        ...
        this.physics.add.collider(
            ...
            () => {
                const factor = SpaceWar.Constants.Ships.WALL_BOUNCE_FACTOR;
                this.playerShip.body.velocity.multiply({
                    x: factor,
                    y: factor,
                });
            }
        ...
        this.events.on(
            ...
            (cfg: ShipState) => {
                ...
                if (SpaceWarClient.playerShipId == cfg?.id) {
    ...
    private _createStellarBodiesLayer(): void {
        ...
        for (var i = 0; i < rooms.length; i++) {
            let room = rooms[i];
            let startTopLeft: Phaser.Math.Vector2 =
                this.getLevel().getMapTileWorldLocation(room.left, room.top);
            let startBottomRight: Phaser.Math.Vector2 =
                this.getLevel().getMapTileWorldLocation(
    ...
    private _showRoom(room: GameRoom): void {
        if (!room.visible) {
            room.visible = true;
            const opponentsInRoom = SpaceWarClient.opponents
                .map((o) => o?.ship)
                .filter((s) => {
                    const shipLoc = s.location;
                    const shipRoom = this.getLevel().getRoomAtWorldXY(
                        shipLoc.x,
                        shipLoc.y
                    );
            ...
            this.add.tween({
                targets: [
                    ...this.getLevel().wallsLayer.getTilesWithin(
                        room.left,
                        room.top,
                        room.width,
                        room.height
                    ),
                    ...this.getLevel().radarLayer.getTilesWithin(
                        room.left,
                        room.top,
                        room.width,
                        room.height
            ...
            opponentsInRoom.forEach((o) => {
                ...
                this.physics.add.collider(o, this.playerShip, () => {
                    if (o?.active && this.playerShip?.active) {
                        const collisionSpeed = o.body.velocity
                            .clone()
                            .subtract(this.playerShip.body.velocity)
                            .length();
                        const damage =
                            collisionSpeed / SpaceWar.Constants.Ships.MAX_SPEED; // maximum damage of 1
    ...
    private _addSupplyCollisionPhysics(options: ShipSupplyOptions): ShipSupply {
        ...
        this.physics.add.collider(supply, this.getLevel().wallsLayer, () => {
            const factor = SpaceWar.Constants.Ships.Supplies.WALL_BOUNCE_FACTOR;
