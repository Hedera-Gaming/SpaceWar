...
export class MultiplayerScene extends BaseScene implements Resizable {
    ...
    override queueGameLevelUpdate(config: GameLevelConfig): BaseScene {
        this._updateGameLevelQueue.splice(
            0,
            this._updateGameLevelQueue.length,
            config
        );
        return this;
    }
    ...
    preload(): void {
        ...
        this.load.audio(
            'background-music',
            `${environment.baseUrl}/assets/audio/space-marine-theme.ogg`
        );
    }

    create(): void {
        SpaceWarClient.mode = 'multiplayer';
        this._gameLevel = null;
        SpaceWarClient.playerShipId = null;
        this._ships.clear();
        ...
        this.addRepeatingAction('high', 'create-stellar-bodies', () => {
            ...
            .addRepeatingAction('high', 'show-hud', () => {
                if (this._shouldShowHud) {
                    ...
                    SpaceWar.game.scene.start(MultiplayerHudSceneConfig.key);
                    SpaceWar.game.scene.bringToTop(
                        MultiplayerHudSceneConfig.key
                    );
            ...
            .addRepeatingAction(
                ...
                (time: number, delta: number) => {
                    ...
                    if (SpaceWarClient.socket?.disconnected) {
    ...
    resize(): void {
        Logging.log(
            'debug',
            `resize called; resetting width, height, background, camera and radar`
        );
    ...
    private async _getPlayerFromServer(): Promise<void> {
        ...
        this.events.on(
            ...
            (cfg: ShipState) => {
                if (cfg.id === SpaceWarClient.playerShipId) {
                    SpaceWarClient.socket?.sendPlayerDeathNotice();
                }
            }
        );

        SpaceWarClient.socket.sendRequestShipRequest();
    }
    ...
    private _createStellarBodiesLayer(): void {
        ...
        const topleft: Phaser.Math.Vector2 =
            this.getLevel().getMapTileWorldLocation(room.left, room.top);
        ...
        const offsetX = 300;
        const offsetY = 300;
        const divisionsX = Math.floor(
            Phaser.Math.Distance.BetweenPoints(
                { x: topleft.x, y: 0 },
                { x: botright.x, y: 0 }
            ) / offsetX
        );
        const divisionsY = Math.floor(
            Phaser.Math.Distance.BetweenPoints(
                { x: 0, y: topleft.y },
                { x: 0, y: botright.y }
            ) / offsetY
    ...
    private _createBackground(): void {
        ...
        this._backgroundStars = this.add.tileSprite(
            this._width / 2,
            this._height / 2,
            this._width * 3,
            this._height * 3,
        ...
        this._backgroundStars.setDepth(
            SpaceWarClient.Constants.UI.Layers.BACKGROUND
        );
    ...
    private _setupCamera(): void {
        ...
        this._camera = new Camera(this, {
            ...
            followObject: this._ships.get(SpaceWarClient.playerShipId),
        });
    ...
    private _playMusic(): void {
        TryCatch.run(
            () =>
                (this._music = this.sound.add('background-music', {
                    loop: true,
                    volume: 0.1,
                }))
        );
    ...
    private _processGameLevelQueue(): void {
        const configs = this._updateGameLevelQueue.splice(
            0,
            this._updateGameLevelQueue.length
        );
    ...
    private _processUpdateShipsQueue(): void {
        TryCatch.run(
            () => {
                const opts = this._updateShipsQueue.splice(
                    0,
                    this._updateShipsQueue.length
                );
                opts.forEach((o) => {
                    ...
                    if (ship) {
                        ...
                        Logging.log(
                            'debug',
                            `creating new PlayerShip ${JSON.stringify(o)}`
                        );
                        ...
                        this.physics.add.collider(
                            ...
                            () => {
                                const factor =
                                    SpaceWar.Constants.Ships.WALL_BOUNCE_FACTOR;
                                ship.body.velocity.multiply({
                                    x: factor,
                                    y: factor,
                                });
                            }
                        ...
                        TryCatch.run(
                            () => this._camera?.ignore(ship.radarSprite),
                            'none'
                        );
                    }
                    ...
                    if (
                        this._needsResize &&
                        ship?.id === SpaceWarClient.playerShipId
                    ) {
            ...
            'debug',
            `error in processing ShipOptions array from Server`,
            'message'
        );
    }
    ...
    private _processRemoveShipsQueue(): void {
        TryCatch.run(
            () => {
                const ids = this._removeShipsQueue.splice(
                    0,
                    this._removeShipsQueue.length
                );
                for (let id of ids) {
                    ...
                    if (ship) {
                        ...
                        if (SpaceWarClient.playerShipId === id) {
                            Logging.log(
                                'info',
                                `player ship removed; queuing game over...`
                            );
                            this.queueEndScene();
                        } else {
                            ship.destroy();
            ...
            'debug',
            `error in processing removeShipsQueue`,
            'message'
        );
    }
    ...
    private _processUpdateSuppliesQueue(): void {
        TryCatch.run(
            () => {
                const opts = this._updateSuppliesQueue.splice(
                    0,
                    this._updateSuppliesQueue.length
                );
                opts.forEach((o) => {
                    ...
                    if (supply) {
                        ...
                        Logging.log(
                            'debug',
                            `creating new ShipSupply ${JSON.stringify(o)}`
                        );
                        switch (o.supplyType) {
                            case 'ammo':
                            ...
                            default:
                                console.warn(
                                    `unknown supplyType of ${o.supplyType} provided`
                                );
                                break;
                        }
            ...
            'debug',
            `error in processing ShipSupplyOptions array from server`,
            'message'
        );
    }
    ...
    private _processRemoveSuppliesQueue(): void {
        TryCatch.run(
            () => {
                const ids = this._removeSuppliesQueue.splice(
                    0,
                    this._removeSuppliesQueue.length
                );
                for (let id of ids) {
            ...
            'debug',
            `error in processing removeSuppliesQueue`,
            'message'
        );
    }
    ...
    private _processFlickerSuppliesQueue(): void {
        TryCatch.run(
            () => {
                const ids = this._flickerSuppliesQueue.splice(
                    0,
                    this._flickerSuppliesQueue.length
                );
                for (let id of ids) {
                    const supply = this.getSupply(id);
                    if (supply) {
            ...
            'debug',
            `error in processing flickerSuppliesQueue`,
            'message'
        );
    }
}
