...
export class GameOverScene extends Phaser.Scene {
    ...
    preload(): void {
        ...
        this.load.audio(
            'game-over-song',
            `${environment.baseUrl}/assets/audio/space-ray.ogg`
        );
    }
    ...
    private _updateScore(): void {
        if (SpaceWarClient.mode === 'multiplayer') {
            const leaderboard = SpaceWar.stats.getLeaderboard();
            this._scoreText.setText([
                'Leaderboard:',
                ...leaderboard
                    .map(
                        (s, i) => `${i + 1}. ${s.name} - ${s.score.toFixed(0)}`
                    )
    ...
    private _createLayout(): void {
        ...
        this._layout.setDepth(SpaceWarClient.Constants.UI.Layers.HUD);
    ...
    private _createScore(): void {
        ...
        const id = SpaceWarClient.playerShipId;
    ...
    private _createMenuItems(): void {
        ...
        const restartButton: TextButton = new TextButton(this, {
            ...
            onHover: () => {
                restartButton
                    .setText({ style: { color: '8d8d8d' } })
                    .setBackground({
                        fillStyle: { color: 0x80ff80, alpha: 1 },
                    });
            },
            onClick: () => {
                switch (SpaceWarClient.mode) {
                    case 'singleplayer':
        ...
        const flex = new FlexLayout(this, {
            width: this._width,
            padding: 10,
            contents: [restartButton, returnToMenuButton],
        });
        flex.setDepth(SpaceWarClient.Constants.UI.Layers.HUD);
    ...
    private _createMusic(): void {
        TryCatch.run(
            () =>
                (this._music = this.sound.add('game-over-song', {
                    loop: true,
                    volume: 0.1,
                }))
        );
