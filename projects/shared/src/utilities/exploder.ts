...
export class Exploder {
    ...
    emitSupplies(shipOpts: ShipState): Array<ShipSupplyOptions> {
        ...
        const fuelContainersCount = Phaser.Math.RND.between(
            1,
            remainingFuel / SpaceWar.Constants.Ships.MAX_FUEL_PER_CONTAINER
        );
        for (var i = 0; i < fuelContainersCount; i++) {
            const amount =
                remainingFuel > SpaceWar.Constants.Ships.MAX_FUEL_PER_CONTAINER
                    ? SpaceWar.Constants.Ships.MAX_FUEL_PER_CONTAINER
                    : remainingFuel;
        ...
        const ammoContainersCount = Phaser.Math.RND.between(
            1,
            remainingAmmo /
                SpaceWar.Constants.Ships.Weapons.MAX_AMMO_PER_CONTAINER
        );
