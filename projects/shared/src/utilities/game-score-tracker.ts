...
export namespace GameScoreTracker {
...
export class GameScoreTracker extends DynamicDataStore<GameScoreTracker.GameStats> {
    ...
    shotLanded(shotFiredBy: string, targetId: string, damage: number): void {
        ...
        if (stats) {
            ...
            Logging.log('debug', { shotFiredBy }, 'hit', { targetId }, 'for', {
                damage,
            });
            this.updateStats({
                shipId: shotFiredBy,
    ...
    opponentDestroyed(destroyerShipId: string, destroyedShipId: string): void {
        ...
        if (stats) {
            const destroyed: Array<GameScoreTracker.Destroyed> =
                stats.opponentsDestroyed;
            const index = destroyed.findIndex(
                (d) => d.targetId === destroyedShipId
            );
            // if target not already listed then add it to list of destroyed
            if (index < 0) {
                Logging.log('info', { destroyerShipId }, 'destroyed', {
                    destroyedShipId,
                });
                destroyed.push({
                ...
                this.updateStats({
                    shipId: destroyerShipId,
                    opponentsDestroyed: destroyed,
                });
            }
        }
    }
    ...
    getLeaderboard(): Array<GameScoreTracker.UserScore> {
        ...
        for (const stats of this.select()) {
            if (stats && stats.name && !highScores.has(stats.name)) {
                ...
                const lowToHighScores = userStats
                    .map((s) => this.getScore(s.shipId))
                    .sort((a, b) => a - b);
                const highestScore = lowToHighScores.pop();
                highScores.set(stats.name, {
                    name: stats.name,
                    score: highestScore,
                });
            }
        }
        const lowToHigh = Array.from(highScores.values()).sort(
            (a, b) => a.score - b.score
        );
    ...
    getStats(
        query: Partial<GameScoreTracker.GameStats>
    ): GameScoreTracker.GameStats {
        const stats = this.select(query).first;
        return {
            ...stats,
            accuracy: this.getAccuracy(
                stats.shotsFired?.length ?? 0,
                stats.shotsLanded?.length ?? 0
            ),
        };
    }
    ...
    getAccuracy(shotsFired: number, shotsLanded: number): number {
        ...
        const landed = Number.isNaN(Number(shotsLanded))
            ? 0
            : Number(shotsLanded);
        if (fired > 0) {
