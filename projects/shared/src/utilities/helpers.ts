...
export namespace Helpers {
     ...
     */
    export function getHeading(angle: number): Phaser.Math.Vector2 {
        let x: number = Math.cos(deg2rad(angle));
        let y: number = Math.sin(deg2rad(angle));
    ...
    export function vector2(
        ...
        return new Phaser.Math.Vector2(innerX, innerY);
    }
