...
import { SpaceWar } from '../space-war';
...
export class Ship
    ...
    implements HasId, HasLocation, HasState<ShipState>
{
    /** ShipOptions */
    ...
    setCurrentState(state: Partial<ShipState>): this {
        if (this.active) {
            ...
            const newIntegrity =
                state.integrity ?? SpaceWar.Constants.Ships.MAX_INTEGRITY;
            const oldIntegrity = this.integrity ?? 0;
            oldIntegrity > newIntegrity
                ? this.subtractIntegrity(oldIntegrity - newIntegrity)
                : this.addIntegrity(newIntegrity - oldIntegrity);
            const newFuel =
                state.remainingFuel ?? SpaceWar.Constants.Ships.MAX_FUEL;
            const oldFuel = this.remainingFuel ?? 0;
            oldFuel > newFuel
                ? this.subtractFuel(oldFuel - newFuel)
                : this.addFuel(newFuel - oldFuel);
            const newAmmo =
                state.remainingAmmo ??
                SpaceWar.Constants.Ships.Weapons.MAX_AMMO;
            const oldAmmo = this.weapon.remainingAmmo;
            oldAmmo > newAmmo
                ? (this.weapon.remainingAmmo = oldAmmo - newAmmo)
                : this.weapon.addAmmo(newAmmo - oldAmmo);
            const newTemp = state.temperature ?? 0;
            const oldTemp = this.temperature;
            oldTemp > newTemp
                ? this.subtractHeat(oldTemp - newTemp)
                : this.addHeat(newTemp - oldTemp);
        }
        return this;
    }
    ...
    get damageSources(): Array<DamageMetadata> {
        return this._lastDamagedBy.map((d) => {
    ...
    lookAt(target: Phaser.Types.Math.Vector2Like): void {
        if (target) {
            const radians: number = Phaser.Math.Angle.Between(
                target.x,
                target.y,
                this.x,
                this.y
    ...
    get isOverheating(): boolean {
        return (
            this._temperature > SpaceWar.Constants.Ships.MAX_SAFE_TEMPERATURE
        );
    }
    ...
    addFuel(amount: number): void {
        this._remainingFuel += amount;
        if (this._remainingFuel > SpaceWar.Constants.Ships.MAX_FUEL) {
            this._remainingFuel = SpaceWar.Constants.Ships.MAX_FUEL;
        }
    }
    ...
    addIntegrity(amount: number): void {
        this._integrity += amount;
        if (this.integrity > SpaceWar.Constants.Ships.MAX_INTEGRITY) {
            this._integrity = SpaceWar.Constants.Ships.MAX_INTEGRITY;
        }
    }
    ...
    selfDestruct(countdownSeconds: number = 3): void {
        this.destroyAtTime =
            this.scene.game.getTime() + countdownSeconds * 1000;
    }
    ...
    death(emit: boolean = true): void {
        if (this.active) {
            ...
            TryCatch.run(
                ...
                'warn',
                'error destroying ship game object',
                'message'
            );
        }
    }
     ...
     * also applies cooling at a rate of {SpaceWar.Constants.COOLING_RATE}
     * @param delta the number of elapsed milliseconds since last update
     */
    private _checkOverheatCondition(delta: number): void {
        if (this.active) {
            if (this.isOverheating) {
                if (
                    this._temperature > SpaceWar.Constants.Ships.MAX_TEMPERATURE
                ) {
                    this.death(); // we are dead
                    return;
                } else {
            ...
            let amountCooled: number =
                SpaceWar.Constants.Ships.COOLING_RATE_PER_SECOND *
                (delta / 1000);
    ...
    private _createGameObj(options: ShipOptions): void {
        ...
        const radius = SpaceWar.Constants.Ships.RADIUS;
        this.setSize(radius * 2, radius * 2);

        this.scene.physics.add.existing(this);
        this.body.setCircle(radius);
        this.body.setMass(options.mass ?? SpaceWar.Constants.Ships.MASS);
        this.body.setBounce(
            SpaceWar.Constants.Ships.BOUNCE,
            SpaceWar.Constants.Ships.BOUNCE
        );
        this.body.setMaxSpeed(SpaceWar.Constants.Ships.MAX_SPEED);
