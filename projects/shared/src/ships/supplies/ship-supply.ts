...
export abstract class ShipSupply
    extends Phaser.GameObjects.Container
    implements HasId, HasState<ShipSupplyOptions>
{
    private _id: string;
    ...
    constructor(scene: BaseScene, options: ShipSupplyOptions) {
        ...
        const radius = SpaceWar.Constants.Ships.Supplies.RADIUS;
        this.setSize(radius * 2, radius * 2);
        this.scene.physics.add.existing(this);
        this.body.setMass(SpaceWar.Constants.Ships.Supplies.MASS);
        this.body.setCircle(radius);
        this.body.setBounce(
            SpaceWar.Constants.Ships.Supplies.BOUNCE,
            SpaceWar.Constants.Ships.Supplies.BOUNCE
        );
    ...
    setCurrentState(config: ShipSupplyOptions): this {
        ...
        const heading = Helpers.vector2(
            Phaser.Math.RND.realInRange(-1, 1),
            Phaser.Math.RND.realInRange(-1, 1)
        );
        const velocity =
            config.velocity || heading.multiply({ x: speed, y: speed });
