...
export class Bullet
    ...
    implements BulletOptions, HasId, HasLocation
{
    readonly id: string;
    ...
    constructor(scene: BaseScene, options: BulletOptions) {
        ...
        SpaceWar.stats.shotFired(this.weapon.ship.id);
    ...
    onShipCollision(ship: Ship): void {
        if (ship.active) {
            ...
            if (remainingIntegrity <= 0) {
                SpaceWar.stats.opponentDestroyed(this.weapon.ship.id, ship.id);
            }
    ...
    get locationInView(): Phaser.Math.Vector2 {
        return Helpers.convertLocToLocInView(
            { x: this.x, y: this.y },
            this.scene
        );
    }
    ...
    private _setInMotion(): void {
        ...
        let deltaV: Phaser.Math.Vector2 = heading.multiply(
            Helpers.vector2(this.force)
        );
    ...
    private _addCollisionDetection(): void {
        ...
        this.scene.physics.add.collider(
            this,
            this.scene.getLevel().wallsLayer,
            () => {
                this.destroy();
            }
        );
