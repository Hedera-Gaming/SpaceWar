...
export class AiController extends InputController {
    ...
    get view(): Phaser.Geom.Polygon {
        const viewDistance = this.isAggro()
            ? 500
            : 500 - this.scene.getShips().length * 2; // less ships = longer view distance
        return Helpers.getView(
            this.ship.location,
            this.ship.rotationContainer.angle,
            viewDistance
        );
    }
    ...
    update(time: number, delta: number): void {
            ...
            if (attacker) {
                ...
                if (this.canSee(attacker.location)) {
                    ...
                    if (this.hasLastKnown()) {
                        ...
                        // yes has last known...
                        if (
                            this._goToWorldLocation(
                                this._lastKnownEnemyLocation
                            )
                        ) {
                            // is at last known?
                            // yes so clear it
        ...
        if (
            this._medPriUpdateAt >=
            SpaceWar.Constants.Timing.MED_PRI_UPDATE_FREQ
        ) {
            this._medPriUpdateAt = 0;
        }
        if (
            this._lowPriUpdateAt >=
            SpaceWar.Constants.Timing.LOW_PRI_UPDATE_FREQ
        ) {
    ...
    canSee(target: Phaser.Types.Math.Vector2Like): boolean {
        if (this._canCheckView) {
            ...
            return (
                ...
                !this.scene
                    .getLevel()
                    .isWallObscuring(this.ship.location, target)
            );
        }
        return false;
    }
    ...
    canUseWeapons(): boolean {
        ...
        if (this._nextWeaponsFireAt == null) {
            this._nextWeaponsFireAt =
                now +
                this.scene.getShips().length * 50 +
    ...
    canUseEngines(): boolean {
        const now = this.scene.time.now;
        if (
            this._nextThrusterFireAt == null ||
            this._nextThrusterFireAt <= now
        ) {
            return true;
        }
        return false;
    ...
    private _setState(state: AiState): void {
        ...
        if (this._previousState != this._state) {
            Logging.log(
                'debug',
                `ship: '${this.ship.name}' state changed from '${this._previousState}' to '${this._state}'`
            );
        }
    }
    ...
    private _patrol(): void {
        ...
        if (this._patrolPath.length < 1) {
            ...
            const { width, height, left, top } = this.scene
                .getLevel()
                .getRoomAtWorldXY(shipLoc.x, shipLoc.y);
            const randTileXInRoom = Phaser.Math.RND.between(
                left + 2,
                left + width - 2
            );
            const randTileYInRoom = Phaser.Math.RND.between(
                top + 2,
                top + height - 2
            );
            const worldLoc = this.scene
                .getLevel()
                .tileToWorldXY(randTileXInRoom, randTileYInRoom);
            const pathTileLocations = this._getPathTo(worldLoc);
            this._patrolPath.splice(
                0,
                this._patrolPath.length,
                ...pathTileLocations
            );
        }
     ...
     */
    private _goToWorldLocation(
        worldXY: Phaser.Types.Math.Vector2Like
    ): boolean {
        const tile = this.scene
            .getLevel()
            .getTileAtWorldXY(worldXY.x, worldXY.y);
        if (tile) {
    ...
    private _goToTileLocation(tileXY: Phaser.Types.Math.Vector2Like): boolean {
        if (this.scene.getLevel().isWithinTile(this.ship.location, tileXY)) {
            ...
            const worldXY = this.scene
                .getLevel()
                .getMapTileWorldLocation(tileXY.x, tileXY.y);
    ...
    private _useEngines(): boolean {
        ...
        if (this.canUseEngines()) {
            ...
            this._nextThrusterFireAt =
                now +
                this.scene.getShips().length * 10 +
                Phaser.Math.RND.realInRange(0, 500);
            return true;
        }
        return false;
    }
