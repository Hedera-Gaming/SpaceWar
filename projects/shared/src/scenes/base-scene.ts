...
export abstract class BaseScene extends Phaser.Scene {
    ...
    private _medPriElapsed: number =
        SpaceWar.Constants.Timing.MED_PRI_UPDATE_FREQ;
    private _lowPriElapsed: number =
        SpaceWar.Constants.Timing.LOW_PRI_UPDATE_FREQ;
    private _ultraLowPriElapsed: number =
    ...
    update(time: number, delta: number): void {
        ...
        // as fast as possible
        Array.from(this._highPriorityActions.values()).forEach((a) =>
            TryCatch.run(() => a(time, delta), 'warn')
        );

        // 30 fps
        if (
            this._medPriElapsed >= SpaceWar.Constants.Timing.MED_PRI_UPDATE_FREQ
        ) {
            this._medPriElapsed = 0;
            Array.from(this._mediumPriorityActions.values()).forEach((a) =>
                TryCatch.run(() => a(time, delta), 'warn')
            );
        }
        ...
        // 15 fps
        if (
            this._lowPriElapsed >= SpaceWar.Constants.Timing.LOW_PRI_UPDATE_FREQ
        ) {
            this._lowPriElapsed = 0;
            Array.from(this._lowPriorityActions.values()).forEach((a) =>
                TryCatch.run(() => a(time, delta), 'warn')
            );
        }
        ...
        // 1 fps
        if (
            this._ultraLowPriElapsed >=
            SpaceWar.Constants.Timing.ULTRALOW_PRI_UPDATE_FREQ
        ) {
            this._ultraLowPriElapsed = 0;
            Array.from(this._ultraLowPriorityActions.values()).forEach((a) =>
                TryCatch.run(() => a(time, delta), 'warn')
            );
        }
    }
