...
export class BattleRoyaleScene extends BaseScene {
    ...
    create(): void {
        ...
        for (let i = 0; i < SpaceWarServer.Constants.Rooms.MAX_BOTS; i++) {
            this.createBot();
        }
        ...
        this.addRepeatingAction('medium', 'send-ships-update', () => {
            ...
            if (shipsState) {
                SpaceWarServer.io.sendUpdatePlayersEvent(
                    this.ROOM_NAME,
                    shipsState
                );
            }
        ...
        this.addRepeatingAction('low', 'send-supplies-update', () => {
            ...
            if (suppliesState.length) {
                SpaceWarServer.io.sendUpdateSuppliesEvent(
                    this.ROOM_NAME,
                    suppliesState
                );
            }
        ...
        this.addRepeatingAction('ultralow', 'send-stats-update', () => {
            ...
            SpaceWarServer.io.sendUpdateStatsToRoom(
                this.ROOM_NAME,
                SpaceWar.stats.getAllStats()
            );
        })
            ...
            .addRepeatingAction('ultralow', 'send-game-level-update', () => {
                SpaceWarServer.io.sendUpdateGameLevelToRoom(
                    this.ROOM_NAME,
                    this.getLevel().currentState
                );
            })
            .addRepeatingAction('ultralow', 'purge-disconnected-users', () => {
                const users = SpaceWarServer.users.delete({
    ...
    createShip(data: SpaceWar.UserData, config?: Partial<ShipState>): Ship {
        ...
        if (this._ships.add(ship)) {
            SpaceWar.stats.start(ship.currentState);
            ...
            SpaceWarServer.users.update({ shipId: ship.id }, data);
    ...
    createBot(): void {
        const botData: SpaceWar.UserData = {
        ...
        let name = `bot-${index}`;
        while (
            SpaceWarServer.users.size({ name: name }) > 0 ||
    ...
    addPlayerToScene(player: SpaceWarServer.UserData): void {
        const user = SpaceWarServer.users.select(player).first;
        if (user) {
            Logging.log(
                'info',
                'adding player:',
            ...
            SpaceWarServer.io.joinRoom(user.socketId, this.ROOM_NAME);
            SpaceWarServer.users.update(user);
            SpaceWarServer.io.sendJoinRoomResponse(user.socketId);
        }
    }

    removePlayerFromScene(player: SpaceWarServer.UserData): void {
        const user = SpaceWarServer.users.select(player).first;
        if (user) {
            Logging.log(
                'info',
            ...
            user.shipId = null;
            SpaceWarServer.users.update(user);
            SpaceWarServer.io.leaveRoom(user.socketId, this.ROOM_NAME);
        }
    }
    ...
    private _setupSceneEventHandling(): void {
        ...
        this.events
            .on(SpaceWar.Constants.Events.SHIP_DEATH, (state: ShipState) => {
                Logging.log(
                    'debug',
                    `received '${SpaceWar.Constants.Events.SHIP_DEATH}' event in scene`
                );
                this._removeShip(state);
            })
            .on(
                SpaceWar.Constants.Events.WEAPON_FIRING,
                (id: string, firing: boolean) => {
                    this.getShip(id)?.weapon?.setEnabled(firing);
                    SpaceWarServer.io.sendWeaponFiringEventToRoom(
                        this.ROOM_NAME,
                        id,
                        firing
                    );
            ...
            .on(
                SpaceWar.Constants.Events.ENGINE_ON,
                (id: string, enabled: boolean) => {
                    this.getShip(id)?.engine?.setEnabled(enabled);
                    SpaceWarServer.io.sendEngineOnEventToRoom(
                        this.ROOM_NAME,
                        id,
                        enabled
                    );
    ...
    private _removeShip(state: ShipState): void {
        ...
        SpaceWarServer.io.sendShipDestroyedEventToRoom(
            this.ROOM_NAME,
            state.id
        );
        ...
        if (this._ships.size({ id: state.id }) > 0) {
            ...
            const user = SpaceWarServer.users.select({
                shipId: state.id,
            }).first;
            if (user) {
                this.removePlayerFromScene(user);
    ...
    private _addSupplyCollisionPhysics(
        ...
        for (let opts of options) {
            ...
            this.physics.add.collider(
                ...
                () => {
                    const factor =
                        SpaceWar.Constants.Ships.Supplies.WALL_BOUNCE_FACTOR;
    ...
    private _addPlayerCollisionPhysicsWithSupplies(ship: Ship): void {
        this.physics.add.collider(
            ...
            (shipGameObj, supplyGameObj) => {
                ...
                SpaceWarServer.io.sendRemoveSuppliesEvent(s.id);
    ...
    private _processRemoveSupplyQueue(...supplyIds: Array<string>): void {
        ...
        if (removeSupplies.length) {
            SpaceWarServer.io.sendRemoveSuppliesEvent(
                this.ROOM_NAME,
                ...removeSupplies
            );
        }
    ...
    private _processFlickerSupplyQueue(): void {
        ...
        if (flickerSupplies.length) {
            ...
            SpaceWarServer.io.sendFlickerSuppliesEvent(
                this.ROOM_NAME,
                ...flickerSupplies
            );
        }
